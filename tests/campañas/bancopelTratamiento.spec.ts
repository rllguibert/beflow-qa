  ...
  let todoPage;   
    ...
    test.beforeEach(async ({ page }) => {
     ...
     // const lowFinalFlow = await page.getByText('¡Eres un Deportista Financiero en Formación! 🏃‍♂️📚💰');
      ...
      await page.getByText('¿En qué situaciones podría usarla?', { exact: true }).click();
      await page.getByText('¿Cuál son los costos asociados para acceder a ella?', { exact: true }).click();
     ...
     // await expect(lowFinalFlow).toContainText(['¡Eres un Deportista Financiero en Formación! 🏃‍♂️📚💰']);  
      ...
      test('Tratamiento_Medium_Financial_test', async ({ page }) => {
        ...
        await page.locator('#selectionBox').getByRole('button', { name: 'Zoom image' }).first().click();
        await page.getByText('Aprender sobre mi Disposición de Efectivo 💰').click();
        await page.getByText('¿En qué situaciones podría utilizarla?', { exact: true }).click();
        await page.getByText('¿Cuáles son los costos asociados a ella?').click();
        await page.getByText('¿Cuál es el monto al que puedo acceder?').click();
        await page.getByText('¿Cómo puedo acceder a ella?').click();
        await page.waitForTimeout(30000);
    ...
    test('Tratamiento_High_Financial_test', async ({ page }) => {
